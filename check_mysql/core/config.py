...
def write_default_config(
    ...
    The template ships a ready-to-edit ``[mysql]`` section and a commented
    ``[ssh]`` section enabling the tunnel once uncommented. Overwrite handling
    (ConfigurationError without ``force``) comes from :func:`write_config`.
    """
...
def render_config(mysql: MySQLConfig, ssh: Optional[SSHConfig] = None) -> str:
    ...
    The ``[ssh]`` section is written active when tunnel settings are given,
    and shipped commented (ready to enable) otherwise.
    """
    lines: list[str] = [
        "# check_mysql configuration — generated by `check_mysql init`.",
        "",
